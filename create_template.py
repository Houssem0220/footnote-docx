...
<w:endnotes xmlns:wpc="http://schemas.microsoft.com/office/word/2010/wordprocessingCanvas"
    ...
    # Add EndnoteReference style
    en_ref_style = etree.SubElement(styles_root, "{%s}style" % w_ns)
    en_ref_style.set("{%s}type" % w_ns, "character")
    en_ref_style.set("{%s}styleId" % w_ns, "EndnoteReference")
    etree.SubElement(en_ref_style, "{%s}name" % w_ns).set("{%s}val" % w_ns, "endnote reference")
    etree.SubElement(en_ref_style, "{%s}basedOn" % w_ns).set("{%s}val" % w_ns, "DefaultParagraphFont")
    en_ref_rPr = etree.SubElement(en_ref_style, "{%s}rPr" % w_ns)
    etree.SubElement(en_ref_rPr, "{%s}vertAlign" % w_ns).set("{%s}val" % w_ns, "superscript")

    # Add EndnoteText style
    en_text_style = etree.SubElement(styles_root, "{%s}style" % w_ns)
    en_text_style.set("{%s}type" % w_ns, "paragraph")
    en_text_style.set("{%s}styleId" % w_ns, "EndnoteText")
    etree.SubElement(en_text_style, "{%s}name" % w_ns).set("{%s}val" % w_ns, "endnote text")
    etree.SubElement(en_text_style, "{%s}basedOn" % w_ns).set("{%s}val" % w_ns, "Normal")
    en_text_pPr = etree.SubElement(en_text_style, "{%s}pPr" % w_ns)
    spacing_en = etree.SubElement(en_text_pPr, "{%s}spacing" % w_ns)
    spacing_en.set("{%s}after" % w_ns, "0")
    spacing_en.set("{%s}line" % w_ns, "240")
    spacing_en.set("{%s}lineRule" % w_ns, "auto")
    en_text_rPr = etree.SubElement(en_text_style, "{%s}rPr" % w_ns)
    etree.SubElement(en_text_rPr, "{%s}sz" % w_ns).set("{%s}val" % w_ns, "20")
    etree.SubElement(en_text_rPr, "{%s}szCs" % w_ns).set("{%s}val" % w_ns, "20")
